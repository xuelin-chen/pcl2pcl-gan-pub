...
MODEL_FILE = os.path.join(ROOT_DIR, 'part_seg', FLAGS.model+'.py')
...
def train():
    with tf.Graph().as_default():
        ...
        parameter_num = int(np.sum([np.prod(v.shape.as_list()) for v in tf.trainable_variables()]))
...
def train_one_epoch(sess, ops, train_writer):
    ...
    for batch_idx in range(num_batches):
        ...
        aug_data = batch_data
        aug_label = batch_label
        #aug_data, aug_label = provider.sort_point_clouds_XYZ_with_label(aug_data, aug_label, sorting_order='random')
        aug_data = provider.rotate_point_cloud_with_normal(aug_data)
        #aug_data[:,:,0:3] = provider.jitter_point_cloud(aug_data[:,:,0:3])
        feed_dict = {ops['pointclouds_pl']: aug_data,
                     ops['labels_pl']: aug_label,
