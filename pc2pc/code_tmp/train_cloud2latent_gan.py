...
from latent_gan import CloudLatentGAN, LatentCodeDataset

# paras for autoencoder on all chairs
para_config = {
    'exp_name': 'debug_cloud_latent_gan',
...
TRAIN_DATASET = shapenet_pc_dataset.ShapeNetPartPointsDataset(para_config['point_cloud_dir'], batch_size=para_config['batch_size'], npoint=para_config['point_cloud_shape'][0])
...
def train():
    with tf.Graph().as_default():
        with tf.device('/gpu:'+str(0)):
            latent_gan = CloudLatentGAN(para_config)
        ...
        for i in range(para_config['epoch']):
            ...
            while TRAIN_DATASET.has_next_batch():
                input_cloud_cur = TRAIN_DATASET.next_batch_noise_added(noise_mu=para_config['noise_mu'], noise_sigma=para_config['noise_sigma'])
                real_code_cur = latent_code_dataset.next_batch()
                feed_dict={
                           latent_gan.noise_cloud_pl: input_cloud_cur,
            ...
            if i % para_config['save_interval'] == 0:
                # save model
