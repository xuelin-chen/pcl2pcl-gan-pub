...
para_config = {
    'exp_name': 'latent2latent_gan',
    'A_ltcode_pkl_filename': '/workspace/pointnet2/pc2pc/run/log_ae_emd_chair_2048_noisy_test_good/latent_codes.pickle',
    'B_ltcode_pkl_filename': '/workspace/pointnet2/pc2pc/run/log_ae_emd_chair_2048_test_good/latent_codes.pickle',
...
B_latent_code_dataset = LatentCodeDataset(para_config['B_ltcode_pkl_filename'],
                                             batch_size=para_config['batch_size'],
                                             shuffle=para_config['shuffle_lcode_dataset'])
...
def train():
    with tf.Graph().as_default():
        ...
        for i in range(para_config['epoch']):
            ...
            while A_latent_code_dataset.has_next_batch() and B_latent_code_dataset.has_next_batch():
                noise_cur = A_latent_code_dataset.next_batch()
                real_code_cur = B_latent_code_dataset.next_batch()
                feed_dict={
            ...
            A_latent_code_dataset._reset()
            B_latent_code_dataset._reset()
