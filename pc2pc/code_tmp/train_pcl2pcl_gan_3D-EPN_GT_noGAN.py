...
para_config_gan = {
    ...
    'recover_ckpt': '/workspace/pointnet2/pc2pc/run_3D-EPN/run_chair/pcl2pcl_wGT_noGAN/log_chair_pcl2pcl_gan_3D-EPN_GT_noGAN_emd_2019-03-14-16-45-22/ckpts/model_3000.ckpt',

    'batch_size': 24,
    'lr': 0.0001,
    'beta1': 0.5,
    'epoch': 3001,
    'k': 0, # train k times for D each loop when training #NOTE: not training GAN part
...
LOG_DIR = os.path.join('run_3D-EPN', 'run_%s'%(cat_name), 'pcl2pcl_wGT_noGAN', 'log_' + para_config_gan['exp_name'] + '_' + para_config_gan['loss'] + '_' + datetime.now().strftime('%Y-%m-%d-%H-%M-%S'))
print(LOG_DIR)
...
def train():
    with tf.Graph().as_default():
        with tf.device('/gpu:'+str(0)):
            ...
            G_loss, G_tofool_loss, reconstr_loss, D_loss, D_fake_loss, D_real_loss, fake_clean_reconstr, eval_loss = latent_gan.model_wGT_noGAN()
        ...
        with tf.Session(config=config) as sess:
            ...
            for i in range(para_config_gan['epoch']):
                ...
                if i % para_config_gan['output_interval'] == 0:
                    ...
                    # save currently generated for debug
