...
# return shape is (N, P, K, 2)
# last dim is 2: batch index, point index
...
def sort_points(points, indices, sorting_method):
    ...
    if sorting_method.startswith('c'):
        ...
        sorting_data = tf.random_uniform([batch_size, point_num, k], dtype=tf.float32) # (N, P, K)
    else:
        print('Unknown sorting method!')
        exit()
...
def sort_point_clouds_XYZ(point_clouds, sorting_order='xyz'):
    '''
    input point clouds: B,N,... - np array
    sort the points in each point cloud
    '''
    eps = 1e-8
    xyz = point_clouds[:,:,0:3]
    normal = point_clouds[:,:,3:]
    pts_min = np.amin(xyz, axis=1, keepdims=True) # 
    pts_max = np.amax(xyz, axis=1, keepdims=True)
    pts_normed = (xyz - pts_min) / (pts_max - pts_min + eps) # (B, N, 3)

    scaling_factors = np.array([math.pow(100.0, 3 - sorting_order.find('x')),
                                math.pow(100.0, 3 - sorting_order.find('y')),
                                math.pow(100.0, 3 - sorting_order.find('z'))])

    sorting_data = np.sum(pts_normed * scaling_factors, axis=-1, keepdims=False) # (B, N)

    sorting_indices = np.argsort(sorting_data, axis=-1)

    pts_sorted = np.zeros(point_clouds.shape)
    for i, indices in enumerate(sorting_indices):
        pts_sorted[i] = point_clouds[i, indices, :]
    return pts_sorted
...
def batch_normalization(data, is_training, name, reuse=None):
    return tf.layers.batch_normalization(data, momentum=0.99,
...
def instance_normalization(data, name, reuse=None):
    return tf.contrib.layers.instance_norm(data, scope=name)
...
def separable_conv2d(input, output, name, is_training, kernel_size, depth_multiplier=1,
    ...
    conv2d = tf.layers.separable_conv2d(input, output, 
...
def depthwise_conv2d(input, depth_multiplier, name, is_training, kernel_size,
                     reuse=None, with_bn=True, activation=tf.nn.elu):
    conv2d = tf.contrib.layers.separable_conv2d(input, num_outputs=None, 
...
# original defines
'''
def conv2d(input, output, name, is_training, kernel_size, strides=(1,1), 
           reuse=None, with_bn=True, activation=tf.nn.elu):
    conv2d = tf.layers.conv2d(input, output, 
                              kernel_size=kernel_size, 
...
def conv2d_transpose(input, output, name, is_training, kernel_size, strides, reuse=None, with_bn=True, activation=tf.nn.elu):
    conv2d = tf.layers.conv2d_transpose(input, output, 
                                                kernel_size=kernel_size, 
                                                strides=strides, 
                                                padding='VALID',
                                                activation=activation,
...
def conv1d(input, output, name, is_training, kernel_size, strides=1, 
           reuse=None, with_bn=True, activation=tf.nn.elu):
    conv1d = tf.layers.conv1d(input, output, 
                              kernel_size=kernel_size, 
                              strides=strides, padding='VALID',
                              activation=activation,
                              kernel_initializer=tf.truncated_normal_initializer(stddev=0.02),
                              kernel_regularizer=tf.contrib.layers.l2_regularizer(scale=1.0),
                              reuse=reuse, name=name, use_bias=not with_bn)
    return batch_normalization(conv1d, is_training, name + '_bn', reuse) if with_bn else conv1d
...
if __name__=='__main__':
    ...
    _, indices = knn_indices_general(qrs, pts, 5, True)
    indices = sort_points(pts, indices, 'cxyz')
    points = tf.gather_nd(pts, indices)

    indices = tf.constant([[0,1],[0,2]])
    slice_qrs = qrs[indices]
    ...
    with tf.Session() as sess:
        ...
        qrs_val = sess.run(slice_qrs)
        print(qrs_val.shape)
'''
if __name__=='__main__':
    points = np.array([[[1,3,3, 1,1,1],[4,8,6, 2,2,2],[4,8,9, 3,3,3],[1,4,1, 4,4,4],[13,8,15, 5,5,5]],
                       [[1,3,3, 1,1,1],[4,8,6, 2,2,2],[4,8,9, 3,3,3],[1,4,1, 4,4,4],[13,8,15, 5,5,5]],
                       [[1,3,3, 1,1,1],[4,8,6, 2,2,2],[4,8,9, 3,3,3],[1,4,1, 4,4,4],[13,8,15, 5,5,5]]])
    print(points.shape)

    a = sort_point_clouds_XYZ(points)
    print(a)
