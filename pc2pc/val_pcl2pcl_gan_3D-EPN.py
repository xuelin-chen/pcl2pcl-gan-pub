...
if cat_name == 'chair':
    para_config_gan['3D-EPN_val_point_cloud_dir'] = os.path.join(config.EPN_dataset_train_dir, '03001627/point_cloud')
    ...
    para_config_gan['3D-EPN_val_point_cloud_dir'] = os.path.join(config.EPN_dataset_train_dir, '04379243/point_cloud')
    ...
    para_config_gan['3D-EPN_val_point_cloud_dir'] = os.path.join(config.EPN_dataset_train_dir, '02691156/point_cloud')
    ...
    para_config_gan['3D-EPN_val_point_cloud_dir'] = os.path.join(config.EPN_dataset_train_dir, '02958343/point_cloud')
...
NOISY_TEST_DATASET = shapenet_pc_dataset.ShapeNet_3DEPN_PointsDataset(para_config_gan['3D-EPN_val_point_cloud_dir'], batch_size=para_config_gan['batch_size'], npoint=para_config_gan['point_cloud_shape'][0], shuffle=False, split='val', preprocess=False)
...
def prepare4test():
    ...
    para_config_gan['LOG_DIR'] = os.path.join('test_3D-EPN', 'val_pcl2pcl_%s'%(cat_name), 'all_models_ShapeNetV1-GT', 'log_test_' + para_config_gan['exp_name'] + '_' + model_name + '_' + datetime.now().strftime('%Y-%m-%d-%H-%M-%S'))
...
if __name__ == "__main__":
    ...
    # test models
    for model_idx in range(500, 3001, 10):
